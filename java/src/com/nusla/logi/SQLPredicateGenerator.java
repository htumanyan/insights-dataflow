	...
	public void generatePredicates(LogiPluginObjects10 rdObjects) 
	{
		try
			...
			for(String kvString: kvStrings)
			{
				...
				{
					sqlColumnName = kv[0];
					requestParamName = kv[1];
					operand = " = ";
				}
				else if(kv.length == 3)
				{
					sqlColumnName = kv[0];
					requestParamName = kv[2];
					if(kv[1].equals("eq")) operand = " = "; 
					else if(kv[1].equals("lt")) operand = " < "; 
					else if(kv[1].equals("gt")) operand = " > "; 
					else if(kv[1].equals("gte")) operand = " >= "; 
					else if(kv[1].equals("lte")) operand = " <= "; 
					else throw new Exception("illegal operand:"+kv[1]);
					
				}
				else
				{
					throw new Exception("illegal mapping:"+kvString);
				}
				...
				else
				{
					...
					if(!paramValue.equals("") && !paramValue.equals("null"))
					{
						...
						predicateString.append(operand);
