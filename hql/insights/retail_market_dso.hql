...
create table if not exists dso_sold_daily_tmp as select 
 ...
 group by is_certified, veh_segment, make, model, geo_dma_id, modelyear,   DD.dt  order by DD.dt  desc;
...
group by  veh_segment, is_certified, make, model, modelyear, geo_dma_id, DD.dt;
...
dso.dt = ds.sales_date
dos.is_certified = ds.is_certified
dso.veh_segment=ds.veh_segment
