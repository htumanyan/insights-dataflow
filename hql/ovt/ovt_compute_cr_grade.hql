...
create  table if not exists make_model_gross_txn	 as select s.reg_key, s.make, s.model, s.model_year, s.ts, sum(gross_txn_flg) over (partition by s.make, s.model order by unix_timestamp(s.ts) range between 3888000 PRECEDING and current row) as cnt from (select model_year, model, make, reg_key, case gross_txn_flg when   1 then  1 else 0 end as gross_txn_flg,  reg_ts as ts from ovt_make_model   order by ts desc ) s;

create  table if not exists make_model_sum_offered_count as select s.reg_key, s.make, s.model, s.model_year, s.ts, sum(s.offered_cnt) over (partition by s.make, s.model order by unix_timestamp(s.ts) range between 3888000 PRECEDING and current row) as cnt from (select make as make, model as model,reg_key,  model_year as model_year, offered_cnt, reg_ts as ts from ovt_make_model order by ts desc ) s;

create  table if not exists make_model_sum_offerng_flg	 as select s.reg_key, s.make, s.model, s.model_year, s.ts, sum(s.offrng_flg) over (partition by s.make, s.model order by unix_timestamp(s.ts) range between 3888000 PRECEDING and current row) as cnt from (select model_year, model, make,reg_key,  reg_ts  as ts, case offrng_flg when 1 then 1 else 0 end as offrng_flg  from ovt_make_model order by ts desc ) s;
...
create  table make_model_metrics as select m1.reg_key,  m1.make, m1.model, m1.model_year, m1.ts, m1.cnt/m3.cnt as effectiveness, m1.cnt/m2.cnt as efficiency, m1.cnt as sum_gross_txn, m2.cnt as sum_offered_cnt, m3.cnt as sum_offerng_flg, m4.sum_pur_amt as sum_pur_amt, m4.sum_sale_nat_mmr as sum_sale_nat_mmr, m4.sum_pur_amt/m4.sum_sale_nat_mmr as mmr_retention 
