...
drop table if exists chrome.engine_build_files;

CREATE EXTERNAL TABLE engine_build_files
(
...
drop table if exists chrome.engine_cache;

CREATE EXTERNAL TABLE engine_cache
(
 input_row_num BIGINT,
 passthru_id INT,
 engine_type_id INT,
...
ROW FORMAT DELIMITED FIELDS TERMINATED BY ','
STORED AS TEXTFILE
LOCATION '/data/database/chrome/engine/cache';
